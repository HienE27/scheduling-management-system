...
/**
 * MySQL/MariaDB implementation of {@link DatabaseCompatibilityHelper}.
 *
 * <p>Active when {@code spring.datasource.driver-class-name=com.mysql.cj.jdbc.Driver}.
 * Emits MySQL-specific SQL: {@code INSERT IGNORE}, {@code CAST(... AS UNSIGNED)},
 * multi-table {@code DELETE alias FROM ...}.
 */
@Repository
@ConditionalOnProperty(name = "spring.datasource.driver-class-name",
        havingValue = "com.mysql.cj.jdbc.Driver", matchIfMissing = true)
public class DbCompatMySqlImpl implements DatabaseCompatibilityHelper {
    ...
    public int insertCompensationDayIfAbsent(Integer staffId, Integer periodId,
        ...
        return em.createNativeQuery(
                "INSERT IGNORE INTO compensation_day " +
    ...
    public int findMaxStaffCodeNumber(String prefix, int prefixLen) {
        Object result = em.createNativeQuery(
                "SELECT COALESCE(MAX(CAST(SUBSTRING(staff_code, :prefixLen + 1) AS UNSIGNED)), 0) " +
